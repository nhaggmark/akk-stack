...
-- Companion death handler: clear recruitment cooldown and reset progression data.
-- Fires immediately when a companion dies, before the death despawn timer.
-- Equipment (companion_inventories) is preserved — only progression resets.
-- Clearing the cooldown bucket ensures the player can re-recruit immediately
-- after the companion returns home (no 15-minute lockout to work around).
function event_death(e)
    if not e.self:IsCompanion() then return end

    -- nil-guard: GetOwnerCharacterID is Companion-only; Lua_NPC cast drops it
    ...
    -- Clear recruitment cooldown so re-recruitment after death is immediate.
    -- On failure the cooldown is set for 15 minutes, which would block the player
    -- from re-recruiting their companion after death if a prior failure happened.
    local cooldown_key = "companion_cooldown_" .. npc_type_id .. "_" .. char_id
    eq.delete_data(cooldown_key)

    -- Reset progression data: XP, level (back to recruited_level), kills, times_died.
    -- Companion "loses their memories" on death but keeps equipment.
    -- equipment is in companion_inventories (separate table), untouched here.
    -- Condition: is_dismissed = 0 catches the active record (C++ sets is_suspended
    -- AFTER NPC::Death() returns, so at event_death time is_suspended is still 0).
    local db = Database()
    local stmt = db:prepare(
